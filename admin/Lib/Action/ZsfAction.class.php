...
class ZsfAction extends Action {
    ...
    private $all=array(
      ...
      'multicorrect'=>array(),
  		...
  		'jn'=>array('tb_grd ON f_zsf_grdid=grdid','tb_dm ON f_zsf_dmid=dmid'),//NB
      //自己的全部+f的显示的东西
  		'para'=>array('zsfid'=>'zsfID','f_zsf_grdid'=>'年级','grdnm'=>'年级名称','f_zsf_dmid'=>'住宿','dmnm'=>'住宿名称','zsfsm'=>'住宿费数目'),//NB
  		//抛去不是zabojin的属性针对para
      'notself'=>array('grdnm','dmnm'),
       ##########modify 添加修改中不需要展示和理会的属性 针对para
      'no_update'=>array('zsfid','grdnm','dmnm'),
      #####update的时候允许为空的值 针对zabojin刨掉不然显示的update字段后
      'allowempty'=>array(),

      'hide_fld'=>array('zsfid','f_zsf_grdid','f_zsf_dmid'),//NB
      'hide_cdt'=>array('zsfid','grdnm','dmnm'),//NB
    ...
    //   'odr_dflt'=>array('bbbnm'=>'ASC'),
      ...
      //hide的fld必须有，他们虽然不显示但是必须选择，这样才能在第一次进入query的时候，隐藏属性可以被调用，特别是id和fid
  		'fld_dflt'=>array('zsfid','f_zsf_grdid','grdnm','f_zsf_dmid','dmnm','zsfsm'),//NB
  		'cdt_dflt'=>array(),//NB
  		...
  		'defaultls'=>1,//默认枚举//NB
  		##########view
  		'no_view'=>array('zsfid','f_zsf_grdid','f_zsf_dmid'),

...
class StatAction extends Action {
    ...
    private $all=array(
    	'mdmk'=>'Stat',//NB
  		...
  		'jn'=>array(),//NB
      //自己的全部+f的显示的东西
  		'para'=>array('statid'=>'STATID','statnm'=>'状态名称','statactvt'=>'状态是否激活','statmk'=>'状态标识'),//NB
  		//抛去不是zabojin的属性针对para
      'notself'=>array(),
       ##########modify 添加修改中不需要展示和理会的属性 针对para
      'no_update'=>array('statid'),
      #####update的时候允许为空的值 针对zabojin刨掉不然显示的update字段后
      'allowempty'=>array(),

      'hide_fld'=>array('statid'),//NB
      'hide_cdt'=>array('statid'),//NB
    ...
    //   'odr_dflt'=>array('aanm'=>'ASC'),
      ...
      //hide的fld必须有，他们虽然不显示但是必须选择，这样才能在第一次进入query的时候，隐藏属性可以被调用，特别是id和fid
  		'fld_dflt'=>array('statid','statnm','statactvt','statmk'),//NB
  		'cdt_dflt'=>array(),//NB
  		...
  		'defaultls'=>1,//默认枚举//NB
  		##########view
  		'no_view'=>array('statid'),
	   
      #########删除提醒
      'deleteconfirm'=>'确定要删除此条记录？',
      #####转义
      'transmean'=>array('statactvt'=>array('0'=>'否','1'=>'是'),'statmk'=>array('1'=>'招生','2'=>'教务')),//NB
      #####默认值
      'dfltvalue'=>array('statactvt'=>1,'statmk'=>1),

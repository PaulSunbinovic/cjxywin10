...
class XxxsAction extends Action {
    ...
    private $all=array(
    	'mdmk'=>'Xxxs',//NB
  		...
  		'jn'=>array(),//NB
      //自己的全部+f的显示的东西
  		'para'=>array('xxxsid'=>'xxxsID','xxxsnm'=>'学习形式名称'),//NB
  		//抛去不是zabojin的属性针对para
      'notself'=>array(),
       ##########modify 添加修改中不需要展示和理会的属性 针对para
      'no_update'=>array('xxxsid',),
      #####update的时候允许为空的值 针对zabojin刨掉不然显示的update字段后
      'allowempty'=>array(),

      'hide_fld'=>array('xxxsid'),//NB
      'hide_cdt'=>array('xxxsid'),//NB
  		
    //   'spccdtls'=>array('spccdt_0'=>array('xxxsid<>0','xxxsID不为0【废话只是测试】')),
  		// 'odrls'=>array('xxxsnm'),
    //   'spccdt_dflt'=>array('spccdt_0'),
    //   'odr_dflt'=>array('xxxsnm'=>'ASC'),
      ...
      //hide的fld必须有，他们虽然不显示但是必须选择，这样才能在第一次进入query的时候，隐藏属性可以被调用，特别是id和fid
  		'fld_dflt'=>array('xxxsid','xxxsnm'),//NB
  		'cdt_dflt'=>array(),//NB

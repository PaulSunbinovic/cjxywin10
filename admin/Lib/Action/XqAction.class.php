...
class XqAction extends Action {
    ...
    private $all=array(
    	'mdmk'=>'Xq',//NB
  		...
  		'jn'=>array(),//NB
      //自己的全部+f的显示的东西
  		'para'=>array('xqid'=>'XQID','xqnm'=>'学期名称','xqdq'=>'学期是否当前'),//NB
  		//抛去不是zabojin的属性针对para
      'notself'=>array(),
       ##########modify 添加修改中不需要展示和理会的属性 针对para
      'no_update'=>array('xqid'),
      #####update的时候允许为空的值 针对zabojin刨掉不然显示的update字段后
      'allowempty'=>array(),

      'hide_fld'=>array('xqid'),//NB
      'hide_cdt'=>array('xqid'),//NB
    ...
    //   'odr_dflt'=>array('aanm'=>'ASC'),
      ...
      //hide的fld必须有，他们虽然不显示但是必须选择，这样才能在第一次进入query的时候，隐藏属性可以被调用，特别是id和fid
  		'fld_dflt'=>array('xqid','xqnm','xqdq'),//NB
  		'cdt_dflt'=>array(),//NB
      ...
      #####转义
      'transmean'=>array('xqdq'=>array('0'=>'否','1'=>'是')),//NB
      #####默认值
      'dfltvalue'=>array('xqdq'=>0),

...
class RcAction extends Action {
    ...
    private $all=array(
    	'mdmk'=>'Rc',//NB
  		...
  		'jn'=>array(),//NB
      //自己的全部+f的显示的东西
  		'para'=>array('rcid'=>'rcID','rcnm'=>'民族名称'),//NB
  		//抛去不是zabojin的属性针对para
      'notself'=>array(),
       ##########modify 添加修改中不需要展示和理会的属性 针对para
      'no_update'=>array('rcid',),
      #####update的时候允许为空的值 针对zabojin刨掉不然显示的update字段后
      'allowempty'=>array(),

      'hide_fld'=>array('rcid'),//NB
      'hide_cdt'=>array('rcid'),//NB
  		
    //   'spccdtls'=>array('spccdt_0'=>array('rcid<>0','rcID不为0【废话只是测试】')),
  		// 'odrls'=>array('rcnm'),
    //   'spccdt_dflt'=>array('spccdt_0'),
    //   'odr_dflt'=>array('rcnm'=>'ASC'),

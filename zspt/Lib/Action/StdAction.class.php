...
class StdAction extends Action {
    ...
    //   'odr_dflt'=>array('bbbnm'=>'ASC'),
      ...
      ###example
      'linkage'=>array('f_stdxqpmj_pmjid'=>array('f_std_grdid'=>2,'f_pmj_bxxsid'=>1,'f_pmj_ccid'=>0,'f_pmj_klid'=>0,'f_pmj_xxxsid'=>0,'f_pmj_zsfwid'=>0,'f_pmj_xzid'=>0,'f_std_sttid'=>1),'f_stdxqpcls_pclsid'=>array('f_std_grdid'=>2,'f_stdxqpmj_xqid'=>1,'f_std_sttid'=>1)),//如果ccc的变换需要转换成2015_ccc 就是2  否则如果是1 非2情况下联动没有值，不允许下拉，3联动不造成下拉有无
    ...
    public function add(){
      header("Content-Type:text/html; charset=utf-8");
      $std=D('Std');$grd=D('Grd');$bxxs=D('Bxxs');$zssz=D('Zssz');$stat=D('Stat');
      $all=$this->all;
      $arr_zsszo=$zssz->getzsszo();$zsszo=$arr_zsszo['data'];
      $all['dfltvalue']['f_std_sttid']=1;
      $all['dfltvalue']['f_std_grdid']=$zsszo['f_zssz_grdid'];
      $all['dfltvalue']['f_stdxqpmj_xqid']=$zsszo['f_zssz_xqid'];
      $all['dfltvalue']['stdpt']=C('PUBLIC').'/img/usr/default.jpg';
      ...
      //重置一些字段
      $arr_bxxsls=$bxxs->getbxxslsbyzs();$bxxsls=$arr_bxxsls['data'];
      $this->assign('f_pmj_bxxsid',$bxxsls);
      #########
      $arr_grdls=$grd->getgrdlsbygrdid($zsszo['f_zssz_grdid']);$grdls=$arr_grdls['data'];
      $this->assign('f_std_grdid',$grdls);
      #########
      $arr_statls=$stat->getstatlsbyzs();$statls=$arr_statls['data'];
      $this->assign('f_std_statid',$statls);
